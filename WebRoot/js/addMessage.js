$(document).ready(function() {
			...
			$.ajax({  
			       type : "POST",
					url : "brand/getAllNoRepeat.action",
			        async: false,
					dataType : "json",
					success : function(data) {  
						$("#test_data2").bsSuggest({
				            ...
				            multiWord: false,
				            showHeader: true,       //显示多个字段的表头
				            effectiveFields: ['brand_name','producing_area','brand_eng'],
				            effectiveFieldsAlias:{brand_name: "品牌名称",producing_area: "品牌产地", brand_eng: "品牌英文"},   
				            data: {
				                'value': data.rows
				            }
						}).on('onSetSelectValue', function (e, keyword) {  
				        	if(data!=null){
				                  $("#brand").val(keyword.key); 
				                }
					        });
					}  
			});
			...
			$.ajax({  
			       type : "POST",
					url : "model/getAllNoRepeat.action",
			        async: false,
					dataType : "json",
					success : function(data) {  
						$("#test_data3").bsSuggest({
						    idField: "factory_code",  
						    keyField: "factory_code",
				            twoWayMatch: true,
				            multiWord: false,
				            showHeader: true, 
				            effectiveFields: ['factory_code'],
				            effectiveFieldsAlias:{factory_code: "机型"},   
				            data: {
				                'value': data.rows
				            }
						}).on('onSetSelectValue', function (e, keyword) {  
				        	if(data!=null){
				                  $("#model").val(keyword.key); 
				                }
					        });
					}  
			});
			
			$("input.send_type:radio").on("change", function() {
				var str = $("input:radio:checked").val();
				if(str == "new" || str == "apply" || str == "purchase") {
					swal("注意事项","禁止使用本系统存储、处理、传输涉及国家秘密、公司机密的文件资料，禁止谈论价格、新技术、政治等敏感信息！","warning");
				}
			})
		});
