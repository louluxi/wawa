...
public class ExcelUitl {
	...
	public 	static <T> void exportExcelToServer(List<T> list,
			LinkedHashMap<String, String> fieldMap, String sheetName,
			HttpServletResponse response,String filePath, String fileName){
			try {
				File f=new File(filePath + File.separator + fileName + ".xls");  
	            f.createNewFile();  
	...
	public static <T> List<T> excelToList(InputStream in, String sheetName,
		...
		try {
			...
			for (String cnName : fieldMap.keySet()) {
				if (!excelFieldList.contains(cnName.trim())) {
					isExist = false;
					break;
				}
			}
			...
			for (int i = 1; i < realRows; i++) {
				...
				if(nullCols==uniqueFields.length&&nullCols !=0){
			...
			for(int i=1;i<realRows;i++){
				...
				for(Entry<String, String> entry : fieldMap.entrySet()){
					...
					//根据中文字段名获取列
					int col = colMap.get(cnNormalName.trim());
					//获取当前单元格的内容 去空格
		...
		if (field != null) {
			...
			if (String.class == fieldType) {
				...
				if(!"".equals(fieldValue)){
					field.set(o, new SimpleDateFormat("yyyyMMdd").parse(fieldValue.toString()));
				}
			} else {
	...
	private static <T> void fillSheet(WritableSheet sheet, List<T> list,
		...
		for (int index = firstIndex; index <= lastIndex; index++) {
			...
			for (int i = 0; i < enFields.length; i++) {
				...
				if("start_date".equals(enFields[i])||"end_date".equals(enFields[i])){
					if(!"".equals(objValue)&&!"null".equals(objValue)&&null != objValue){
						SimpleDateFormat sf1 = new SimpleDateFormat("EEE MMM dd hh:mm:ss z yyyy", Locale.ENGLISH); 
						Date date = sf1.parse(objValue.toString());
						SimpleDateFormat sf2 = new SimpleDateFormat("yyyy-MM-dd");
						objValue = sf2.format(date);
					}
				}
...
//		beanEntity e = new beanEntity();
//		e.setLine("line");
//		e.setCondition("condition");
//		list.add(e);
//		OutputStream out = new FileOutputStream(new File("E:\\1111111111.xls")); 
//		ExcelUitl.listToExcel(list, fieldMap, "test", out);
	}
}
